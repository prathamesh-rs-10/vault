...
options.add_argument("--headless")  # Run in headless mode, useful for running without a UI
...
# Find the Profit-Loss section using Selenium
try:
    data = driver.find_element(By.ID, "profit-loss")
    tdata = data.find_element(By.TAG_NAME, "table")
    table_data = []
    
    for row in tdata.find_elements(By.TAG_NAME, "tr"):
        row_data = [cell.text.strip() for cell in row.find_elements(By.TAG_NAME, "th") + row.find_elements(By.TAG_NAME, "td")]
    ...
    logging.error(f"Error scraping the Profit-Loss section: {e}")
